...
In the vast majority of cases, the default space unit should be used as the base for any dimensional size (e.g. height, padding, margin, width) which needs to alter in proportion to text zoom. Pixels should only be specified as the unit in instances where a dimension doesn't alter, for example borders and keylines.
